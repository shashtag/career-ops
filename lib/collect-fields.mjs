...
export const COLLECT_EXPRESSION = `(() => {
  // Option lists are capped so a 250-country dropdown cannot flood context. The
  // cap is lossy, so every capped list is flagged — see optionsTruncated below.
  const OPTION_CAP = 60;
  ...
  for (const el of nodes) {
    ...
    if (el.tagName === 'SELECT') {
      ...
      optionsTruncated = all.length > OPTION_CAP;
      options = all.slice(0, OPTION_CAP);
