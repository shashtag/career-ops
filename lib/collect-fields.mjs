 ...
 *                options, optionsTruncated, valueLength, valueTruncated }
 */
...
export const COLLECT_EXPRESSION = `(() => {
  ...
  // value is capped too, but it is the one field whose cap cannot be silent:
  // lib/answer-sanitizer.mjs checks it against the field's own maxLength and a
  // 2200-char "did a whole report section land here" warning, and both read the
  // length — so a quietly truncated value would make an over-long answer pass
  // the last gate before submit as clean. The true length rides along in
  // valueLength, and valueTruncated tells the audit its content checks saw only
  // a prefix. The cap sits far above the 2200-char warn on purpose: a real
  // answer never reaches it, so valueTruncated fires only on the pathological
  // case it exists for.
  ...
  for (const el of nodes) {
    ...
    if (type === 'checkbox' || type === 'radio') {
      ...
      if (el.checked) {
        groups[key].value = groups[key].value
          ...
          : (optText || 'checked');
        // Keep valueLength truthful: the audit reads it in place of value.length.
        groups[key].valueLength = groups[key].value.length;
      }
      continue;
    }
    ...
    out.push({
      label: clean(labelFor(el)).slice(0, LABEL_CAP),
      value: fullValue.slice(0, VALUE_CAP),
      valueLength: fullValue.length,
      valueTruncated: fullValue.length > VALUE_CAP,
      type,
