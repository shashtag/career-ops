 ...
 * @param {{multiline?: boolean, required?: boolean, maxLength?: number,
 *          valueLength?: number, valueTruncated?: boolean}} [opts]
 *
 * `valueLength` is the field's TRUE length when `value` reached us capped (the
 * collector bounds it so one pathological field cannot spend an agent's whole
 * context). The length checks below must use it, not value.length, or a capped
 * value reports a length it does not have and an over-long answer passes this
 * gate — the last one before submit — as clean. Absent, value.length stands.
...
export function auditAnswer(label, value, opts = {}) {
  ...
  // True length, which is only the same as v.length when nothing capped the value.
  const len = Number.isFinite(opts.valueLength) ? opts.valueLength : v.length;

  if (opts.maxLength && len > opts.maxLength) {
    add('over-max-length', 'error', `${len} chars exceeds field max of ${opts.maxLength}`);
  }

  if (len > 2200) {
    add('suspiciously-long', 'warn', `${len} chars — check a whole report section did not land here`);
  }
  ...
  // Every check above scans `v`, so a capped value means they saw a prefix. The
  // length checks are already exact via valueLength; the content ones are not,
  // and a miss past the cap would otherwise read as a clean field.
  if (opts.valueTruncated) {
    add('value-truncated', 'warn',
      `content checks read the first ${v.length} of ${len} chars — inspect the field itself before submitting`);
  }
