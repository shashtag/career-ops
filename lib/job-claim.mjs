...
// CLI: node lib/job-claim.mjs <list|parked|sweep|claim ID [--resume|--hold-tab]|release ID>
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (cmd === 'list') console.log(JSON.stringify(activeClaims(openUrls), null, 2));
    ...
    console.log('usage: node lib/job-claim.mjs <list|parked|sweep|claim ID [--resume|--hold-tab]|release ID>');
    process.exit(2);
  }
}
