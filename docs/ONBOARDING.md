...
The step-by-step cold-start script. `AGENTS.md` keeps the gate — run
`node doctor.mjs --json` on the first message of each session — and sends you
here only when `onboardingNeeded` is true, which is almost never after the first
run (#3601).

Work these in order. Nothing here is optional once onboarding has started: the
system scores offers against whatever these files say, so a half-filled profile
produces confidently wrong evaluations.
