...
**Jurisdiction derivation (same as the #2026 series siblings):** run `node jurisdiction-lookup.mjs --table protected-grounds --json` — it derives the candidate's jurisdiction from their location in `config/profile.yml` and returns the matching rows from `templates/protected-grounds.yml`. Verdict `none` → skip this step entirely; no guessing, no applying another jurisdiction's list. Verdict `UNCERTAIN`, or exit 2 → derive the jurisdiction and read the table by hand, exactly as before. The lookup performs the key match only — every judgement below, `legitimate_contexts` included, stays yours. The table is a data reference: every row carries a `legal_basis`, per-ground `legitimate_contexts` where they exist, `sources`, and an `as_of` verification date.
