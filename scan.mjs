...
const KNOWN_FLAGS = [
  ...
  '--since', '--quiet', '--json', '--timeout', '--help', '-h',
];
...
const VALUE_FLAGS = ['--company', '--posted-after', '--posted-before', '--since', '--timeout'];
...
async function main() {
  ...
  if (!dryRun && verifiedOffers.length > 0) {
    if (viableOffers.length > 0) {
      await appendToPipeline(viableOffers);
      await appendToScanHistory(viableOffers, date);
    }
    if (cappedOffers.length > 0) {
      await appendToScanHistory(cappedOffers, date, 'skipped_cap');
